...
exports.configureHook = function (hook, defaults, overwrite, root) {
    ...
    var packageJsonContent = Fs.readFileSync(packagePath, { encoding: 'utf8' });
    var parts = internals.getPackageParts(packageJsonContent);
    var package = JSON.parse(parts.content);
    ...
    else if (!package.hasOwnProperty(hook) || overwrite) {
        package[hook] = newHookValues;
        Fs.writeFileSync(packagePath, parts.head + JSON.stringify(package, null, 2) + parts.tail, { encoding: 'utf8' });
    }
};
...
// preserve whatever whitespace there is in the package.json
// outside of the opening/closing curly brackets
internals.getPackageParts = function (packageContent) {

    var regex = /^(\s*)({[\s\S]*})(\s*)$/m;
    var matches = packageContent.match(regex);
